...
Note that the first two terms of $C_2(s)$ are almost the same as $C_1(s)$. Why is the third term $0.055/(s+20)$ negligible? The factor $0.055$ is small, but that is not the main reason for the third term to be small. To see what is happening, let's do the inverse LTs:
...
$$
  \tilde G(s) = \frac{1}{(s+1)(s+2)}.
$$

_Note_: Note that we have picked the gain of $\tilde G(s)$ so that $G(s)$ and $\tilde G(s)$ have the same DC gain.
:::

:::{#exm-dominant-pole-approximation-2}
Consider the system
$$
  G(s) = \frac{10}{(s+1)(s+4)(s+10)}.
$$
In this case, we cannot ignore any pole. Note that the pole at $-1$ **does not dominate** the pole at $-4$, and the pole at $-4$ **does not dominate** the pole at $-10$.  Thus, we cannot partition the poles into two sets such that one set dominates the other. 
:::
