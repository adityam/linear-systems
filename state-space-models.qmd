...
State space models provide a nicer way to solve such DEs. In particular, we will provide a method to convert general $2$-nd order LDE into a vector equation
\begin{align*}
\dot x(t) &= A x(t) \\
y(t) &= C x(t)
\end{align*}
...
## Convertting DE to SSM

We start with the same 2nd order example used above:
...
Another method to obtain a SSM from the LDE is as follows. Write the LDE \eqref{eq:2nd-DE} as an integro-differential equation:
\begin{align*}
...
We will revisit converting LDEs to SSMs in more details later. First, we present some examples for directly obtaining SSMs for circuits.
...
![A simple circuit](figures/svg/circuits4.svg){#fig-lcr}
...
We want a relationship between input $u(t)$ and output $v_C(t)$. So, we use KCL/KVL to find a relationship between them. In this example, we use KVL:
\begin{align*}
...
## Converting DE/TF to SSM

In the previous sections, we constructed the state space representations "by hand". We now consider a few "automatic" methods of constructing state space representations from a DE (or equivalently a TF).
...
:::{#exm-SSM-CC}
Consider a generic second order TF:
$$ G(s) = \frac{b_1 s+b_0}{s^2 + a_1 s + a_0} $$
In parrticular, we will consider
$$ G(s) = \frac{s+5}{s^2 + 3s + 2} $$
Find the different canconical form representations of this system.
:::
