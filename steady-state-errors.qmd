...
We will also need to find the values of $K$ for which the closed loop system is stable. That is left as an exercise. 
...
We first use Routh-Hurwitz to check that the TF from $R(s)$ to $Y(s)$ (and therefore the TF from $D(s)$ to $Y(s)$) is stable. The redetails are left as an exercise.
