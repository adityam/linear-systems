...
We now compute the eigenvalues. For $λ_1 = -1$, we have
$$
\MATRIX{-6 & 4 \\ -5 & 3 } \MATRIX{ v_{11} \\ v_{12} }
= -1 \MATRIX{ v_{11} \\ v_{12} }
$$
Thus, the eigen-subspace corresponding to $λ_1 = -1$ is:
$$ -5 v_{11} + 4 v_{12} = 0. $$
We can pick any vector in this sub-space as $v_1$. For convenience, we pick
$$ v_1 = \MATRIX{ 4 \\ 5}. $$

Similarly, for $λ_2 = -2$, we have
$$
\MATRIX{-6 & 4 \\ -5 & 3 } \MATRIX{ v_{21} \\ v_{22} }
= -2 \MATRIX{ v_{21} \\ v_{22} }
$$
Thus, the eigen-subspace corresponding to $λ_2 = -2$ is:
$$ -4 v_{11} + 4 v_{12} = 0. $$
We can pick any vector in this sub-space as $v_2$. For convenience, we pick
$$ v_2 = \MATRIX{ 1 \\ 1}. $$
...
$$
\bbox[5pt,border: 1px solid]
{y(t) = C e^{At} x_0 + \int_{0}^t C e^{A (t-τ) } B u(τ) d τ}
$$
